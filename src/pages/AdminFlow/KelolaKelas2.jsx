...
function KelolaKelas2() {
	...
	const getAllCourse = async (pageNumber) => {
		try {
			...
			const response = await axios.get(
				`${BASE_URL}/course/all?page=${pageNumber}`
			);
	...
	const handleEdit = (code) => {
		const selected = course.content.find(
			(courseItem) => courseItem.code === code
		);
	...
	return (
		<>
			<section className="container flex justify-between px-6 my-3">
				<div className="font-Montserrat font-bold text-xl pt-1">
					Kelola Kelas
				</div>
				...
				>
					<ReactPaginate
						...
						previousLinkClassName={
							"text-center items-center rounded-md font-medium flex gap-2 justify-center items-center bg-darkblue-05 w-20 h-8 text-white hover:bg-purple-800 transition-all duration-200"
						}
						nextLinkClassName={
							"text-center items-center rounded-md font-medium flex gap-2 justify-center items-center bg-darkblue-05 w-20 h-8 text-white hover:bg-purple-800 transition-all duration-200"
						}
						activeClassName={
							"border rounded-md w-7 h-7 text-center bg-blue-500 text-white"
						}
						disabledClassName={
							"border rounded-md text-center bg-blue-500 text-black cursor-not-allowed"
						}
					/>
				</nav>
